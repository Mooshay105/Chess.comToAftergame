...
async function main(YYYY: number, MM: number) {
	...
	for (const user of usersToScrape) {
		...
		const games = await getGames(user, YYYY, MM);
...
function runOnThird() {
	...
	if (now.getDate() === config.uploadDay) {
		if (!hasRun) {
			...
			main(now.getMonth() === 0 ? now.getFullYear() - 1 : now.getFullYear(), month).catch(
				console.error,
			);
		} else {
...
setInterval(runOnThird, 60 * 60 * 1000);
